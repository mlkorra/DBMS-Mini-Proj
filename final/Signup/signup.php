<?php
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if(!filter_var($myuserid, FILTER_VALIDATE_EMAIL)){
	      ...
	      $sql = "SELECT mail_id FROM custInfo WHERE mail_id = '$myuserid'";
	        ...
	        //session_register("myusername");
   		    $sql = "INSERT INTO custInfo (mail_id,password,name,phone_no,default_addr) VALUES ('$myuserid','$mypassword','$myusername',$mynumber,'$myaddress')";
   	     	if(mysqli_query($db,$sql)==TRUE) {
   	     		$_SESSION['status']="Signup has been Successfully Completed";
        		 header("location: ../homepages/login.php");
     		}
     		else{
				echo  "<div style='text-align:center'> <font color=red size='4pt'>An error has occurred  while adding user to database</font> </div>";
    	 	}
    	  }
    	  else {
        	$error = "User has already been registered on this email Id";
         	echo "<div align='centre'> <font color=red size='4pt'>User has already been registered on this email Id</font> </div>";
      	  }
     	}

     	else 
     	{
     		echo "<div align='centre'> <font color=red size='4pt'>Passwords Do Not Match</font> </div>";
     	}
   	}
   }
?>
	...
	<h2 class="sub-head">Fill the form to Register </h2>
		<div class="sub-main">
			<form action="" method="post">
				...
				<input  placeholder="Confirm Password" name="pass1" class="pass" type="password" required="">
